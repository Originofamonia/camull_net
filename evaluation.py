...
def evaluate_model(device_in, uuid, ld_helper):
    ...
    for path in glob.glob(srch_path):
        ...
        if not os.path.exists("../graphs/" + uuid):
            os.mkdir("../graphs/" + uuid)
        metrics = get_roc_auc(model, test_dl, figure=True,
                              path="../graphs/" + uuid, fold=fold + 1)
        accuracy, sensitivity, specificity, roc_auc, you_max, you_thresh = [
            *metrics]
        ...
        tot_acc += accuracy
        tot_sens += sensitivity
        tot_spec += specificity
        tot_roc_auc += roc_auc
        fold += 1
        ticks.update()
        tocks.count = 0
...
def get_roc_auc(model_in, test_dl, figure=False, path=None, fold=1):
    ...
    try:
        ...
        print(e)
    metrics = [opt_acc, opt_sens, opt_spec, roc_auc, youdens_s_max,
               optimal_thresh]

    if figure:

        if path is None:
            path = "../graphs/auc-{date:%Y-%m-%d_%H:%M:%S}.png".format(
                date=datetime.datetime.now())
        else:
            # append dir
            path = path + "/auc-fold{}-{date:%Y-%m-%d_%H:%M:%S}.png".format(
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in enumerate(test_dl):
            ...
            for i in range(4):  # hard coded batch size of 4
                ...
                net_out = model_in((batch_X[i].view(-1, 1, 110, 110, 110),
                                    batch_Xb[i].view(1, 21)))
