"""The following script contains functions for normalising and standardising
the input data. Although the training data has already been standardised,
the user inputted data is not. """

from sklearn import preprocessing
import numpy as np

min_max = {"AGE": (54.4, 91.4), "PTEDUCAT": (4, 20), "CDRSB": (0, 17),
           "ADAS11": (0, 59), "ADAS13": (0, 74), "RAVLT_immediate": (1, 75),
           "RAVLT_learning": (-5, 14), "RAVLT_forgetting": (-1035, 15),
           "RAVLT_perc_forgetting": (-9409, 100)}

encodings = {"GENDER": {"Female": 0, "Male": 1},
             "RACCAT": {"Am Indian/Alaskan": 0, "Asian": 1, "Black": 2,
                        "Haiwaiian/Other PI": 3, "More than one": 4,
                        "Unknown": 5, "White": 6}}

column_names_to_normalize = ["AGE", "PTEDUCAT", "CDRSB", "ADAS11", "ADAS13",
...
def min_max_scale(in_dict):
    ...
    for key in in_dict.keys():
        if key in column_names_to_normalize:
            X = in_dict[key]
            X_std = ((X - min_max[key][0]) / (
                        min_max[key][1] - min_max[key][0]))
            X_scaled = X_std * (min_max[key][1] - min_max[key][0]) + \
                       min_max[key][0]
...
def convert_to_np(in_dict):
    """Gets clinical features vector from unormalized user inputted values.
    This would be used during inference to change user inputted values. """
